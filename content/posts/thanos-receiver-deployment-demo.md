...
本文介绍的 Demo 仅供参考，如果想要将其投入生产，还需要：

- 为软租户的 Receiver 配置 TLS 证书校验；
- 上文提到，开启远程写入将增加 Prometheus 的内存使用，我们需要根据 [文档](https://prometheus.io/docs/practices/remote_write/) 中的建议对其参数进行调优；
...
- 部署前应根据指标数据量评估 Receiver 的 Request 、 Limit 以及副本数，防止其因数据量过大而导致 OOM；
