...
在本例中，受害者页面为 VP 4。如果该页面已发生更改，则内核会将其复制回磁盘并修改 PTE 4 中的有效位。随后内核再把 VP 3 从磁盘复制到主存中的 PP 3，更新 PTE 3 中的有效位。下图展示了异常处理程序返回后示例页表的状态：

![20220609113920](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20220609113920.png)

上述在磁盘和主存之间传输页面的活动被称为交换（Swapping）或者分页（Paging）。页面从磁盘传输到主存被称为 Swapping In 或 Paging In，反之则为 Swapping Out 或 Paging Out。尽管我们可以努力预测缺页故障并在引用未缓存页面前分页，但现代系统均采用按需分页（Demand Paging）的策略，即在缺页故障发生后才进行分页。
