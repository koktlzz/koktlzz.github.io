...
- Section：位于 ELF 头和节头表之间，包括：
  ...
  - *.data*：已初始化的全局变量和静态变量。局部变量在运行时位于栈中，不会出现在 *.data* 或 *.bss* 中；
  - *.bss*：未初始化或初始化为 0 的全局变量和静态变量。此 Section 只是一个占位符，在目标文件中不占用实际空间，因此可以提升空间效率。这些变量在运行时被分配到内存中，初始值为零；
  - *.symtab* ：包含在程序中定义和引用的函数和全局变量信息的符号表（Symbol Table）；
