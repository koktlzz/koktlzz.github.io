...
链接器支持单独编译（Separate Compilation），因此在软件开发中起着至关重要的作用。与其将大型应用程序组织为一个单一的大块源文件，不如将其分解为一些更小、更易于管理并且可以单独修改和编译的模块。一旦我们修改了其中的某个模块，只需重新编译该模块并将其链接到应用程序，无需再次编译其他文件。
...
- Section：位于 ELF 头和节头表之间，包括：
  - .text：编译后程序的机器码；
  - .rodata：只读数据，例如`printf`中的格式字符串，Switch 语句的 [跳转表](/posts/machine-level-representation-of-programs-note/#switch) 等；
...
上述三个 Section 仅存在于可重定位目标文件，在可执行目标文件中并不存在。我们可以使用 [READELF](https://man7.org/linux/man-pages/man1/readelf.1.html) 工具阅读目标文件中的内容，[示例程序 main.c](/posts/linking-note/#编译器驱动) 生成的目标文件符号表条目如下：
...
其中，off 是该内存段中第一个 Section 在目标文件中的偏移量，align 是程序头中指定的对齐方式。这种对齐要求是一种优化，它可以使目标文件被更加有效地加载到内存中。
...
其中，`-fpic`指示编译器生成 [与位置无关代码](/posts/linking-note/#与位置无关代码)（Position-Independent Code），而`-shared`则指示链接器创建共享目标文件。一旦共享库文件创建成功，就可以将其链接到示例程序中：
...
现代系统在编译共享库时会生成一种无需重定位即可加载的代码，即与位置无关代码（Position-Independent Code，PIC），这样共享库就能被多个正在运行的进程同时引用。

### PIC 数据引用

无论我们在何处加载目标模块（包括共享目标模块），数据段与代码段之间的距离始终相同。编译器在 PIC 数据段的开头创建了一个全局偏移量表（Global Offset Table，GOT），其中的每个条目都对应一个被目标模块引用的全局变量和函数。编译器还会为这些条目生成重定位记录。加载时，动态链接器重定位每个 GOT 条目，使其包含被引用对象的绝对地址。每个引用了全局变量的目标模块都有自己的 GOT。

下图展示了示例共享库`libvector.so`中的 GOT：

![20220331211838](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20220331211838.png)

函数[`addvec`](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20220317215726.png)通过 GOT[3] 间接加载全局变量`addcnt`的地址，然后令其自增。其关键思想在于下一条指令`addl`的地址（即 %rip）与 GOT[3] 之间的偏移量是一个运行时常量。如果`addcnt`是由另一个共享模块定义的，则需要通过 GOT 间接访问。
