...
假设我们模型中的每个内存地址都有 $m$ 位，则共有 $M = 2^m$ 个唯一地址。如下图所示，缓存是一个包含了 $S = 2^s$ 个缓存集（Cache Set）的数组，每个缓存集由 E 个缓存行（Cache Line）组成。每个缓存行中都有一个包含了 $B = 2^b$ 字节数据的 Block，一位标识该行是否存储了有效信息的有效位，以及一个用于唯一标识行中 Block 的 $t$ 位（$t = m -(b + s)$）标记位（Tag Bits）。
...
该程序具有良好的局部性，但在直接映射缓存中易发生冲突丢失。假设缓存有两个缓存集，每个 Block 可以容纳 16 个字节的数据。因此缓存的容量为 32 字节，可以完整的保存整个数组。数组`x[8]`和`y[8]`中的元素与缓存集的映射关系如下：

![20220110223717](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20220110223717.png)

由于两数组相同索引的元素均映射到相同的缓存集，因此每次循环中的指令读取数组元素时，都会发生冲突缺失。我们将这种情况称为颠簸（Thrashing），即缓存重复加载并驱逐相同缓存集中的 Block。
