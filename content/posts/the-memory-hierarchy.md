...
当程序需要访问存储在第 k + 1 级设备中的数据对象 d 时，会首先在第 k 级设备中查找。如果 d 恰好缓存在第 k 级设备中，那么便称为缓存命中（Cache Hits）。反之则称为缓存缺失（Cache Misses），第 k 级设备会从第 k + 1 级设备中获取 d 所在的 Block。如果此时第 k 级设备已满，则现有的 Block 将会被覆盖（驱逐）。Block 的驱逐策略有多种，比如随机替换，以及 LRU（Least Recently Used）。 LRU 策略会将被访问时间据现在最久远的 Block 驱逐。
