...
series: ["CSAPP 读书笔记"]
tags: ["OS"]
summary: "在使用高级语言，如 C、Java 编程时，我们无法了解程序在机器级别的实现。而使用汇编语言编写程序时，程序员则只能引用低级指令。编译器提供的类型检查有助于检测程序错误，确保我们引用和操作数据的方式一致 ..."
---

在使用高级语言，如 C、Java 编程时，我们无法了解程序在机器级别的实现。而使用汇编语言编写程序时，程序员则只能引用低级指令。编译器提供的类型检查有助于检测程序错误，确保我们引用和操作数据的方式一致。更重要的是，使用高级语言编写的程序可以在多种不同的机器上编译运行，而汇编语言则与机器特性高度相关。
