...
函数`vframe`声明了一个变长的指针数组`*p[n]`，将在栈中占用 8n 个字节的空间。由于 n 的值是函数第一个参数给出的，因此编译器无法确定要为该函数的运行时栈分配多少空间。

为了管理可变大小的栈帧，x86-64 使用寄存器 %rbp 作为帧指针（又称为基指针，base pointer）。栈帧的组织情况如下图所示：

![20211123223334](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20211123223334.png)

我们可以看到先前版本的 %rbp 被保存在栈中（图中的 Saved %rbp），因为它是一个被调用者保存的寄存器。然后在程序的执行过程中，%rbp 将一直指向这个位置。一些固定长度的局部变量，比如`i`，就可以根据其相对于 Saved %rbp 的偏移量来引用。函数编译后生成的部分汇编代码如下：

![20211123224435](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20211123224435.png)

函数首先将 %rbp 压入栈中，然后将其置为当前运行时栈的地址（汇编代码第 2 到 3 行）。接下来函数在栈上分配了 16 个字节的空间，栈指针指向图中 s1 的位置。前 8 个字节用于存储局部变量`i`，后 8 个字节并未使用。然后它开始为数组`*p[n]`分配空间，栈指针指向图中 s2 的位置，它相对于 s1 的偏移量是通过汇编代码第 5 到 7 行计算得到的：

$$(8n + 22) \& (-16) = \begin{cases}
   8n + 8 &\text n 为奇数 \cr
   8n + 16 &\text n 为偶数
\end{cases}$$

由于结果必然是 16 的倍数，因此实现了数据对齐。汇编代码第 8 到 10 行将图中 p 的值置为最接近 s2 的 8 的倍数。假设 n 为 5 或 6，s1 的值为 2065 或 2064，则图中各值为：
