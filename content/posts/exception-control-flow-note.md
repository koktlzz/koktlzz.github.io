...
错误（Faulting）是由一些错误状况引起的异常，而这些错误情况有可能被处理程序修正，否则将返回到内核中的中止例程（图中的`abort`）：

![20220210222537](https://cdn.jsdelivr.net/gh/koktlzz/ImgBed@master/20220210222537.png)

#### 中止

与错误相比，导致中止（Abort）发生的错误状况无法挽救，通常是硬件出现问题，如 RAM 位损坏引起的奇偶校验错误。中止处理程序永远不会将控制权返回给应用程序：
...
- 除法错误：当应用程序尝试除以 0 或除法指令的结果对目标操作数来说太大时，就会发生除法错误。Unix 不会试图纠正除法错误，而是直接中止程序；
- 页面错误：当程序读取的虚拟地址对应的 Page 不在内存而在磁盘上时，将发生页面错误。处理程序将磁盘上适当的虚拟内存页面映射到物理内存页面，然后重新启动出错指令；
- 机器检查：在执行故障指令期间检测到致命的硬件错误时，会发生机器检查，机器检查处理程序永远不会将控制权返回给应用程序。
