...
我们改进存储系统的初衷是希望解决 Series Churn 的问题，而基于 Block 的设计则减少了查询时涉及的 Series 数量。假设使用索引查询的复杂度为 $O(n^2)$，现在我们成功减少了 n 的数量。然而根据“算法 101”，这并不会给我们带来任何好处。如果以前查询性能很差，那么现在依然会很糟糕。

在实践中，大多数查询操作的响应很快。然而只要时间跨度较大，即使只查询几个 Series 也会很慢。我在所有工作开始前的最初想法便是为这个问题找到一个解决方案：我们需要一个更加强大的 [倒排索引](https://en.wikipedia.org/wiki/Inverted_index)。

倒排索引可以基于数据内容的子集来为我们提供快速的查询能力。简而言之，我可以在不遍历全部 Series 的情况下，找到所有包含标签`app="nginx"`的 Series。

为此，我们需要为每个 Series 分配一个独有的 ID。这个 ID 可以在恒定的时间，即 $O(1)$ 内检索出来。在这种情况下，ID 是我们的向前索引（Forward Index）。
