...
另一个艰巨的任务是在数据被删除或更新时修改索引。通常最简单的方法就是在保证数据库可查询且一致的同时，重新计算并重写它们。V3 存储系统为每个 Block 都分配了一个单独的索引。对于已持久化的 Block，其索引只有在 Block 被压缩时才会重写。而对于内存中可变的 Block，其索引则需要持续更新。
