...
- 在多次调用中持续跟踪状态的函数：如伪随机数生成函数 [`rand`](https://man7.org/linux/man-pages/man3/rand.3.html)，其当前调用的结果取决于上次迭代的中间结果。因此如果多个线程调用该函数，我们就无法确定返回的随机数序列。修改此类函数唯一的方法便是重写它们，使其不依赖任何`static`数据并让调用者通过参数来传递状态信息；
